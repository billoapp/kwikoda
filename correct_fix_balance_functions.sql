-- Correct fix for get_tab_balance function
-- Drop and recreate to fix the ambiguous tab_id issue

-- First drop the existing function
DROP FUNCTION IF EXISTS get_tab_balance(uuid);

-- Recreate with proper parameter naming to avoid ambiguity
CREATE OR REPLACE FUNCTION get_tab_balance(tab_id UUID)
RETURNS NUMERIC AS $$
DECLARE
    order_total NUMERIC := 0;
    payment_total NUMERIC := 0;
    balance NUMERIC := 0;
BEGIN
    -- Sum confirmed orders for this tab
    -- Use explicit table alias to avoid ambiguity
    SELECT COALESCE(SUM(o.total), 0) INTO order_total
    FROM tab_orders o
    WHERE o.tab_id = tab_id 
    AND o.status = 'confirmed';
    
    -- Sum successful payments for this tab
    -- Use explicit table alias to avoid ambiguity
    SELECT COALESCE(SUM(p.amount), 0) INTO payment_total
    FROM tab_payments p
    WHERE p.tab_id = tab_id 
    AND p.status = 'completed';
    
    -- Calculate balance (orders - payments)
    balance := order_total - payment_total;
    
    RETURN balance;
END;
$$ LANGUAGE plpgsql;

-- Also fix should_tab_be_overdue if it has similar issues
DROP FUNCTION IF EXISTS should_tab_be_overdue(uuid);

CREATE OR REPLACE FUNCTION should_tab_be_overdue(tab_id UUID)
RETURNS BOOLEAN AS $$
DECLARE
    tab_balance NUMERIC;
    is_overdue BOOLEAN := FALSE;
    bar_business_hours JSONB;
    current_time TIMESTAMPTZ := NOW();
BEGIN
    -- Get current balance
    tab_balance := get_tab_balance(tab_id);
    
    -- Only check overdue status if there's an outstanding balance
    IF tab_balance > 0 THEN
        -- Check if current time is outside business hours
        IF NOT is_within_business_hours(
            (SELECT bar_id FROM tabs WHERE id = tab_id), 
            current_time
        ) THEN
            is_overdue := TRUE;
        END IF;
    END IF;
    
    RETURN is_overdue;
END;
$$ LANGUAGE plpgsql;

-- Test the functions
-- SELECT get_tab_balance('your-tab-id-here'::uuid) as balance;
-- SELECT should_tab_be_overdue('your-tab-id-here'::uuid) as is_overdue;
